...
func printUsage() {
	...
	fmt.Println("  luma-cli auth login [account]    Create an authorization link")
	fmt.Println("  luma-cli auth complete           Finish after user approval")
	fmt.Println("  luma-cli auth login --key <key>  Save API key or legacy card key")
	...
	fmt.Println("  luma-cli avatar-persona create <name>        Create an avatar persona")
	...
	fmt.Println("  luma-cli url-read --url <url>     Read URL content through cloud Jina Reader")
